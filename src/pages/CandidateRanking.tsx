...
import { Search, Filter, Download, Star, CheckCircle, XCircle, ArrowUpDown, ExternalLink } from 'lucide-react';
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1">Review and manage ranked candidates for ABC Tech - Senior AI/ML Engineer.</p>
        </div>
        <div className="flex space-x-3">
          ...
          <button className="flex items-center space-x-2 bg-srm-600 hover:bg-srm-500 text-white px-4 py-2 rounded-lg font-medium transition-colors shadow-sm">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl shadow-sm overflow-hidden flex flex-col">
        ...
        <div className="p-4 border-b border-slate-800 bg-slate-900/50 flex flex-col sm:flex-row gap-4">
          ...
          <div className="flex gap-4">
            <select 
              ...
              className="bg-slate-800 border border-slate-700 text-white rounded-lg px-4 py-2 focus:outline-none focus:ring-1 focus:ring-srm-500 transition-colors"
            >
              <option value="all">All Tiers</option>
              <option value="high">Tier 1 (High Match)</option>
              <option value="medium">Tier 2 (Medium Match)</option>
              <option value="low">Tier 3 (Low Match)</option>
            </select>
            
            <button className="flex items-center justify-center space-x-2 px-4 py-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-lg text-slate-300 transition-colors">
                ...
                <tr key={candidate.id} className="hover:bg-slate-800/50 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <div className="h-10 w-10 rounded-full bg-gradient-to-br from-slate-700 to-slate-600 flex items-center justify-center text-white font-bold border border-slate-600">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-white">{candidate.education.cgpa} CGPA</div>
                    <div className="text-xs text-slate-400">{candidate.education.active_backlogs === 0 ? 'No Backlogs' : `${candidate.education.active_backlogs} Backlogs`}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <Link to={`/candidates/${candidate.id}`} className="text-srm-400 hover:text-srm-300 transition-colors bg-srm-400/10 hover:bg-srm-400/20 px-3 py-1.5 rounded-md inline-flex items-center">
        ...
        <div className="bg-slate-900 px-6 py-4 border-t border-slate-800 flex items-center justify-between">
          ...
          <div className="flex space-x-2">
            <button className="px-3 py-1 border border-slate-700 rounded-md bg-slate-800 text-slate-400 cursor-not-allowed">Previous</button>
            <button className="px-3 py-1 border border-slate-700 rounded-md bg-slate-800 text-slate-400 cursor-not-allowed">Next</button>
          </div>
        </div>
      </div>
    </div>
